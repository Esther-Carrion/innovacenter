 ...
 * Adopted from Dimox
 *
 * @since Catch Adaptive 2.1
 */
class Parallax_Frame_Json_Ld_Schema {
	private $position = 0;
	...
	function parallax_frame_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'parallax-frame' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'parallax-frame' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'parallax-frame' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'parallax-frame' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'parallax-frame' ); // text for an author page
		$text['404']      = __( 'Error 404', 'parallax-frame' ); // text for the 404 page
	...
	function json_ld_breadcrumbs() {
		...
		if ( isset( $options['breadcumb_option'] ) && $options['breadcumb_option'] ) {
			$show_on_home = ( isset( $options['breadcumb_on_homepage'] ) && $options['breadcumb_on_homepage'] ) ? 1 : 0;

			$breadcrumb = $this->parallax_frame_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
...
new Parallax_Frame_Json_Ld_Schema();
